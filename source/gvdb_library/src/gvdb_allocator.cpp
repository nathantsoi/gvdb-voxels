...
Allocator::Allocator() {
  ...
  cudaCheck(cuModuleLoad(&cuAllocatorModule, CUDA_GVDB_COPYDATA_PTX),
            "Allocator", "Allocator", "cuModuleLoad", CUDA_GVDB_COPYDATA_PTX,
            mbDebug);

  cudaCheck(cuModuleGetFunction(&cuFillTex, cuAllocatorModule, "kernelFillTex"),
            "Allocator", "Allocator", "cuModuleGetFunction", "cuFillTex",
            mbDebug);
  cudaCheck(
      cuModuleGetFunction(&cuCopyTexC, cuAllocatorModule, "kernelCopyTexC"),
      "Allocator", "Allocator", "cuModuleGetFunction", "cuCopyTexC", mbDebug);
  cudaCheck(
      cuModuleGetFunction(&cuCopyTexF, cuAllocatorModule, "kernelCopyTexF"),
      "Allocator", "Allocator", "cuModuleGetFunction", "cuCopyTexF", mbDebug);
  cudaCheck(cuModuleGetFunction(&cuCopyBufToTexC, cuAllocatorModule,
                                "kernelCopyBufToTexC"),
            "Allocator", "Allocator", "cuModuleGetFunction", "cuCopyBufToTexC",
            mbDebug);
  cudaCheck(cuModuleGetFunction(&cuCopyBufToTexF, cuAllocatorModule,
                                "kernelCopyBufToTexF"),
            "Allocator", "Allocator", "cuModuleGetFunction", "cuCopyBufToTexF",
            mbDebug);
  cudaCheck(
      cuModuleGetFunction(&cuCopyTexZYX, cuAllocatorModule, "kernelCopyTexZYX"),
      "Allocator", "Allocator", "cuModuleGetFunction", "cuCopTexZYX", mbDebug);
  cudaCheck(cuModuleGetFunction(&cuRetrieveTexXYZ, cuAllocatorModule,
                                "kernelRetrieveTexXYZ"),
            "Allocator", "Allocator", "cuModuleGetFunction", "cuRetrieveTexXYZ",
            mbDebug);
}
...
Allocator::~Allocator() {
  ...
  cudaCheck(cuModuleUnload(cuAllocatorModule), "Allocator", "~Allocator",
            "cuModuleUnload", "cuAllocatorModule", false);
}
...
void Allocator::PoolCreate(uchar grp, uchar lev, uint64 width, uint64 initmax,
  ...
  if (bGPU) {
    size_t sz = p.size;
    cudaCheck(cuMemAlloc(&p.gpu, sz), "Allocator", "PoolCreate", "cuMemAlloc",
              "", mbDebug);
    cudaCheck(cuMemsetD8(p.gpu, 0, sz), "Allocator", "PoolCreate", "cuMemsetD8",
              "", mbDebug);
  }
...
void Allocator::PoolCommit(int grp, int lev) {
  ...
  // p->stride << std::endl;
  cudaCheck(cuMemcpyHtoD(p->gpu, p->cpu, p->lastEle * p->stride), "Allocator",
            "PoolCommit", "cuMemcpyHtoD", "", mbDebug);
}
...
void Allocator::PoolFetch(int grp, int lev) {
  DataPtr *p = &mPool[grp][lev];
  cudaCheck(cuMemcpyDtoH(p->cpu, p->gpu, p->lastEle * p->stride), "Allocator",
            "PoolFetch", "cuMemcpyDtoH", "", mbDebug);
}
...
void Allocator::PoolCommitAtlasMap() {
  ...
  for (int n = 0; n < mAtlasMap.size(); n++) {
    if (mAtlasMap[n].cpu != 0x0) {
      p = &mAtlasMap[n];
      cudaCheck(cuMemcpyHtoD(p->gpu, p->cpu, p->lastEle * p->stride),
                "Allocator", "PoolCommitAtlasMap", "cuMemcpyHtoD", "", mbDebug);
    }
  }
}
  ...
  for (int grp = 0; grp < MAX_POOL; grp++)
    for (int lev = 0; lev < mPool[grp].size(); lev++) {
      ...
      if (mPool[grp][lev].gpu != 0x0)
        cudaCheck(cuMemFree(mPool[grp][lev].gpu), "Allocator", "PoolReleaseAll",
                  "cuMemFree", "", mbDebug);
    }
...
uint64 Allocator::PoolAlloc(uchar grp, uchar lev, bool bGPU) {
  ...
  if (p->lastEle >= p->max) {
    ...
    if (p->gpu != 0x0) {
      size_t sz = p->size;
      CUdeviceptr new_gpu;
      cudaCheck(cuMemAlloc(&new_gpu, sz), "Allocator", "PoolAlloc",
                "cuMemAlloc", "", mbDebug);
      cudaCheck(cuMemsetD8(new_gpu, 0, sz), "Allocator", "PoolAlloc",
                "cuMemsetD8", "", mbDebug);
      cudaCheck(cuMemcpy(new_gpu, p->gpu, p->stride * p->lastEle), "Allocator",
                "PoolAlloc", "cuMemcpy", "", mbDebug);
      cudaCheck(cuMemFree(p->gpu), "Allocator", "PoolAlloc", "cuMemFree", "",
                mbDebug);
      p->gpu = new_gpu;
    }
  }
...
void Allocator::CreateMemLinear(DataPtr &p, char *dat, int stride, uint64 cnt,
  ...
  if (dat == 0x0) {
    if (bCPU) {
      ...
      if (bAllocHost)
        cudaCheck(cuMemAllocHost((void **)&p.cpu, sizeof(float) * 3),
                  "Allocator", "CreateMemLinear", "cuMemAllocHost", "",
                  mbDebug);
      else
  ...
  if (p.gpu != 0x0)
    cudaCheck(cuMemFree(p.gpu), "Allocator", "CreateMemLinear", "cuMemFree", "",
              mbDebug);
  cudaCheck(cuMemAlloc(&p.gpu, p.size), "Allocator", "CreateMemLinear",
            "cuMemAlloc", "", mbDebug);
...
void Allocator::FreeMemLinear(DataPtr &p) {
  if (p.cpu != 0x0)
    free(p.cpu);
  if (p.gpu != 0x0)
    cudaCheck(cuMemFree(p.gpu), "Allocator", "FreeMemLinear", "cuMemFree", "",
              mbDebug);
  p.cpu = 0x0;
  p.gpu = 0x0;
}
...
void Allocator::RetrieveMem(DataPtr &p) {
  cudaCheck(cuMemcpyDtoH(p.cpu, p.gpu, p.size), "Allocator", "RetrieveMem",
            "cuMemcpyDtoH", "", mbDebug);
}

void Allocator::CommitMem(DataPtr &p) {
  cudaCheck(cuMemcpyHtoD(p.gpu, p.cpu, p.size), "Allocator", "CommitMem",
            "cuMemcpyHtoD", "", mbDebug);
}
...
// OpenGL 3D texture
    ...
    cudaCheck(cuSurfObjectCreate(&p.surf_obj, &resDesc), "Allocator",
    ...
    cudaCheck(
        cuTexObjectCreate(&p.tex_obj, &surfReadDesc, &texReadDesc, nullptr),
    ...
    if (res.x > 0 && res.y > 0 && res.z > 0) {
      // Debug: Print texture allocation parameters
      uint64 totalVoxels = uint64(res.x) * uint64(res.y) * uint64(res.z);
      uint64 memoryMB = (totalVoxels * getSize(dtype)) / (1024 * 1024);
      gprintf("DEBUG: Attempting CUDA 3D texture allocation:\n");
      gprintf("  - Dimensions: %d x %d x %d = %llu voxels\n", res.x, res.y,
              res.z, totalVoxels);
      gprintf("  - Data type: %d, Size per voxel: %d bytes\n", dtype,
              getSize(dtype));
      gprintf("  - Total memory: %llu MB\n", memoryMB);
      gprintf("  - Format: %d, Channels: %d\n", desc.Format, desc.NumChannels);

      // Validate CUDA 3D texture limits
      CUdevice dev;
      cuCtxGetDevice(&dev);
      int maxWidth, maxHeight, maxDepth;
      cuDeviceGetAttribute(&maxWidth,
                           CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_WIDTH, dev);
      cuDeviceGetAttribute(&maxHeight,
                           CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_HEIGHT, dev);
      cuDeviceGetAttribute(&maxDepth,
                           CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_DEPTH, dev);

      gprintf("  - GPU 3D texture limits: %d x %d x %d\n", maxWidth, maxHeight,
              maxDepth);

      // Check if dimensions exceed GPU limits
      if (res.x > maxWidth || res.y > maxHeight || res.z > maxDepth) {
        gprintf("ERROR: Requested texture dimensions (%d x %d x %d) exceed GPU "
                "limits (%d x %d x %d)\n",
                res.x, res.y, res.z, maxWidth, maxHeight, maxDepth);
        gprintf("  - Consider reducing atlas dimensions or increasing voxel "
                "size\n");
        return; // Don't attempt allocation
      }

      // Validate descriptor parameters
      if (desc.NumChannels != 1 && desc.NumChannels != 2 &&
          desc.NumChannels != 4) {
        gprintf("ERROR: Invalid NumChannels (%d). Must be 1, 2, or 4\n",
                desc.NumChannels);
        return;
      }

      cudaCheck(cuArray3DCreate(&p.garray, &desc), "Allocator",
                "AllocateTextureGPU", "cuArray3DCreate", "", mbDebug);
      ...
      if (preserve > 0 && old_array != 0) {
        ...
        cudaCheck(cuLaunchKernel(cuFillTex, grid.x, grid.y, grid.z, block.x,
        ...
        if (cp.Depth < desc.Depth)
          cudaCheck(cuMemcpy3D(&cp), "Allocator", "AllocateTextureGPU",
                    "cuMemcpy3D", "preserve", mbDebug);
      }
    ...
    if (old_array != 0)
      cudaCheck(cuArrayDestroy(old_array), "Allocator", "AllocateTextureGPU",
...
void Allocator::AllocateAtlasMap(int stride, Vector3DI axiscnt) {
  ...
  if (q.gpu != 0x0)
    cudaCheck(cuMemFree(q.gpu), "Allocator", "AllocateAtlasMap", "cuMemFree",
              "", mbDebug);
  cudaCheck(cuMemAlloc(&q.gpu, q.size), "Allocator", "AllocateAtlasMap",
            "cuMemAlloc", "", mbDebug);
...
bool Allocator::TextureCreate(uchar chan, uchar dtype, Vector3DI res, bool bCPU,
  ...
  cudaCheck(cuCtxSynchronize(), "Allocator", "TextureCreate",
            "cuCtxSynchronize", "", mbDebug);
...
bool Allocator::AtlasCreate(uchar chan, uchar dtype, Vector3DI leafdim,
  ...
  cudaCheck(cuCtxSynchronize(), "Allocator", "AtlasCreate", "cuCtxSynchronize",
            "", mbDebug);
...
void Allocator::CopyChannel(int chanDst, int chanSrc) {
  ...
  cudaCheck(cuMemcpy3D(&cp), "Allocator", "CopyChannel", "cuMemcpy3D", "",
            mbDebug);
}
...
bool Allocator::AtlasResize(uchar chan, uint64 max_leaf) {
  ...
  gprintf("DEBUG: AtlasResize called with max_leaf=%llu\n", max_leaf);
  gprintf("  - Current atlas brick count: %d x %d x %d = %d\n", axiscnt.x,
          axiscnt.y, axiscnt.z, axiscnt.x * axiscnt.y * axiscnt.z);
  gprintf("  - Brick dimensions: %d^3 (leafdim), apron: %d\n", leafdim,
          p.apron);

  // Expand Z-axis of atlas
  int newZ = int(ceil(max_leaf / float(axiscnt.x * axiscnt.y)));
  gprintf("  - Calculating new Z: ceil(%llu / %d) = %d\n", max_leaf,
          axiscnt.x * axiscnt.y, newZ);

  // Safety check for unreasonable Z dimensions
  // Use a more conservative limit to prevent GPU memory exhaustion
  if (newZ > 50) { // 50 * 24 * 24 = 28,800 bricks max
    gprintf("ERROR: Calculated Z dimension (%d) exceeds safety limit (50)!\n",
            newZ);
    gprintf("  - This would create a %d x %d x %d atlas with %.1f GB memory\n",
            axiscnt.x, axiscnt.y, newZ,
            (float(axiscnt.x * 34) * float(axiscnt.y * 34) * float(newZ * 34) *
             4.0f) /
                (1024 * 1024 * 1024));
...
void Allocator::AllocateNeighbors(uint64 cnt) {
  ...
  if (mNeighbors.gpu != 0x0)
    cudaCheck(cuMemFree(mNeighbors.gpu), "Allocator", "AllocateNeighbors",
              "cuMemFree", "", mbDebug);
  cudaCheck(cuMemAlloc(&mNeighbors.gpu, mNeighbors.size), "Allocator",
            "AllocateNeighbors", "cuMemAlloc", "", mbDebug);
}

void Allocator::CommitNeighbors() {
  cudaCheck(cuMemcpyHtoD(mNeighbors.gpu, mNeighbors.cpu, mNeighbors.size),
            "Allocator", "CommitNeighbors", "cuMemcpyHtoD", "", mbDebug);
}
...
extern "C" CUresult cudaCopyData(cudaArray *dest, int dx, int dy, int dz,
                                 int dest_res, cudaArray *src, int src_res);
...
void Allocator::AtlasCopyTex(uchar chan, Vector3DI val, const DataPtr &src) {
  ...
  case T_UCHAR:
    cudaCheck(cuLaunchKernel(cuCopyTexC, grid.x, grid.y, grid.z, block.x,
                             block.y, block.z, 0, NULL, args, NULL),
              "Allocator", "AtlasCopyTex", "cuLaunch", "cuCopyTexC", mbDebug);
    break;
  case T_FLOAT:
    cudaCheck(cuLaunchKernel(cuCopyTexF, grid.x, grid.y, grid.z, block.x,
                             block.y, block.z, 0, NULL, args, NULL),
...
void Allocator::AtlasCopyLinear(uchar chan, Vector3DI offset,
  ...
  switch (mAtlas[chan].type) {
  case T_UCHAR:
    cudaCheck(cuLaunchKernel(cuCopyBufToTexC, grid.x, grid.y, grid.z, block.x,
                             block.y, block.z, 0, NULL, args, NULL),
              "Allocator", "AtlasCopyLinear", "cuLaunch", "cuCopyBufToTexC",
              mbDebug);
    break;
  case T_FLOAT:
    cudaCheck(cuLaunchKernel(cuCopyBufToTexF, grid.x, grid.y, grid.z, block.x,
                             block.y, block.z, 0, NULL, args, NULL),
              "Allocator", "AtlasCopyLinear", "cuLaunch", "cuCopyBufToTexF",
              mbDebug);
    break;
  };
}
...
void Allocator::AtlasRetrieveTexXYZ(uchar chan, Vector3DI val, DataPtr &dest) {
  ...
  cudaCheck(cuLaunchKernel(cuRetrieveTexXYZ, grid.x, grid.y, grid.z, block.x,
                           block.y, block.z, 0, NULL, args, NULL),
            "Allocator", "AtlasRetrieveXYZ", "cuLaunch", "cuRetrieveTexXYZ",
            mbDebug);
...
void Allocator::AtlasCopyTexZYX(uchar chan, Vector3DI val, const DataPtr &src) {
  ...
  cudaCheck(cuLaunchKernel(cuCopyTexZYX, grid.x, grid.y, grid.z, block.x,
                           block.y, block.z, 0, NULL, args, NULL),
...
void Allocator::AtlasCommitFromCPU(uchar chan, uchar *src) {
  ...
  cudaCheck(cuMemcpy3D(&cp), "Allocator", "AtlasCommitFromCPU", "cuMemcpy3D",
            "", mbDebug);
}
...
void Allocator::AtlasFill(uchar chan) {
  ...
  cudaCheck(cuLaunchKernel(cuFillTex, grid.x, grid.y, grid.z, block.x, block.y,
                           block.z, 0, mStream, args, NULL),
            "Allocator", "AtlasCommitFromCPU", "cuLaunch", "cuFillTex",
            mbDebug);
}

void Allocator::AtlasRetrieveSlice(uchar chan, int slice, int sz,
                                   CUdeviceptr gpu_buf, uchar *cpu_dest) {
  ...
  cp.Depth = 1;
  cudaCheck(cuMemcpy3D(&cp), "Allocator", "AtlasRetrieveSlice", "cuMemcpy3D",
            "", mbDebug);
}

void Allocator::AtlasWriteSlice(uchar chan, int slice, int sz,
                                CUdeviceptr gpu_buf, uchar *cpu_src) {
  ...
  cp.Height = atlasres.y;
  cp.Depth = 1;
  cudaCheck(cuMemcpy3D(&cp), "Allocator", "AtlasWriteSlice", "cuMemcpy3D", "",
            mbDebug);
}
...
void Allocator::AtlasReleaseAll() {
  for (int n = 0; n < mAtlas.size(); n++) {
    ...
    if (mAtlas[n].surf_obj != 0x0) {
      cudaCheck(cuSurfObjectDestroy(mAtlas[n].surf_obj), "Allocator",
    ...
    if (mAtlas[n].tex_obj != 0x0) {
      cudaCheck(cuTexObjectDestroy(mAtlas[n].tex_obj), "Allocator",
    ...
    if (mAtlas[n].garray != 0x0 && mAtlas[n].glid == -1) {
      cudaCheck(cuArrayDestroy(mAtlas[n].garray), "Allocator",
...
// Free opengl memory
  ...
  for (int n = 0; n < mAtlasMap.size(); n++) {
    ...
    if (mAtlasMap[n].gpu != 0x0) {
      cudaCheck(cuMemFree(mAtlasMap[n].gpu), "Allocator", "AtlasReleaseAll",
                "cuMemFree", "AtlasMap", mbDebug);
      mAtlasMap[n].gpu = 0x0;
    }
  }
  mAtlasMap.clear();
...
bool cudaCheck(CUresult launch_stat, const char *obj, const char *method,
...
void StartCuda(int devsel, CUcontext ctxsel, CUdevice &dev, CUcontext &ctx,
               CUstream *strm, bool verbose) {
  // NOTES:
  ...
  for (int n = 0; n < cnt; n++) {
    ...
    int pi;
    cuDeviceGetAttribute(&pi, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_WIDTH,
                         dev_id);
    if (verbose)
      gprintf("Max. texture3D width: %d\n", pi);
    cuDeviceGetAttribute(&pi, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_HEIGHT,
                         dev_id);
    if (verbose)
      gprintf("Max. texture3D height: %d\n", pi);
    cuDeviceGetAttribute(&pi, CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURE3D_DEPTH,
                         dev_id);
    if (verbose)
  ...
  if (devsel == GVDB_DEV_CURRENT) {
    ...
    cudaCheck(cuCtxGetCurrent(&ctx), "(global)", "StartCuda", "cuCtxGetCurrent",
              "GVDB_DEV_CURRENT", false);
    cudaCheck(cuCtxGetDevice(&dev), "(global)", "StartCuda", "cuCtxGetDevice",
              "GVDB_DEV_CURRENT", false);
  }
  if (devsel == GVDB_DEV_EXISTING) {
    //--- Use existing context passed in
    ctx = ctxsel;
    cudaCheck(cuCtxSetCurrent(ctx), "(global)", "StartCuda", "cuCtxSetCurrent",
  ...
  if (devsel >= 0) {
    ...
    cudaCheck(cuDeviceGet(&dev, devsel), "(global)", "StartCuda", "cuDeviceGet",
              "", false);
    cudaCheck(cuCtxCreate(&ctx, CU_CTX_SCHED_AUTO, dev), "(global)",
              "StartCuda", "cuCtxCreate", "", false);
  }
